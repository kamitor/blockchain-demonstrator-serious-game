    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            /*while (true)
            {
                ...
                {
                    Console.WriteLine("-----------------------------");
                    Console.WriteLine($"role: {player.Key} inventory: {player.Value.Inventory} backorder: {player.Value.Backorder}");
                    if (player.Value.IncomingOrder != null)
                    {
                        Console.WriteLine($"incoming order: {player.Value.IncomingOrder.OrderDay}, {player.Value.IncomingOrder.ArrivalDay}, {player.Value.IncomingOrder.Volume}");
                    }

                    foreach (var item in player.Value.IncomingDelivery)
                    {
                        Console.WriteLine($"orderday: {item.OrderDay} arrivalday: {item.ArrivalDay} volume: {item.Volume}");
                    }
                    Console.WriteLine("-----------------------------");
                }
